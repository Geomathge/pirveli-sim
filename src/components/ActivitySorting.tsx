...
export function ActivitySorting() {
  ...
  const initGame = (currentTheme = theme) => {
    ...
    // Generate guaranteed non-overlapping coordinates in the top box
    const total = countA + countB;
    // Guaranteed non-overlapping discrete slots across 2 staggered rows (14 total slots):
    const slotPool: { x: number; y: number }[] = [
      // Row 1 (7 slots, evenly spaced)
      { x: 4, y: 8 },
      { x: 18, y: 8 },
      { x: 32, y: 8 },
      { x: 46, y: 8 },
      ...
      // Row 2 (7 slots, staggered for natural, organic distribution)
      { x: 7, y: 54 },
      { x: 21, y: 54 },
      { x: 35, y: 54 },
      { x: 49, y: 54 },
    ...
    // Shuffle slot pool so items appear in randomized, well-spaced positions
    const shuffledSlots = [...slotPool].sort(() => Math.random() - 0.5);

    const positions: { x: number; y: number }[] = [];
    for (let i = 0; i < total; i++) {
      const slot = shuffledSlots[i % shuffledSlots.length];
      // Subtle organic jitter (±1% x, ±1% y) that guarantees zero overlap
      const jitterX = Math.round((Math.random() * 2 - 1) * 10) / 10;
      const jitterY = Math.round((Math.random() * 2 - 1) * 10) / 10;
      positions.push({
        x: Math.max(3, Math.min(89, slot.x + jitterX)),
        y: Math.max(6, Math.min(56, slot.y + jitterY)),
      });
    }
    ...
    <LayoutGroup id="sorting-pairing-flow">
      ...
      >
        ...
        {isAllSorted && !isExporting && (
          ...
          >
            <div className="flex items-center gap-2 text-sky-800 font-black text-base sm:text-lg">
              <Sparkles className="w-5 h-5 text-amber-500" />
              <span>რომელია მეტი?</span>
            </div>

            <p className="text-xs sm:text-sm font-bold text-slate-500">
              დააკვირდი და მონიშნე, რომელია მეტი:
            </p>

            {/* Choice Buttons for child to mark which is more */}
            <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 w-full pt-1">
              <button
                onClick={() => handleAnswerSelect('left')}
                className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-sm sm:text-base transition-all cursor-pointer border-2 shadow-xs active:scale-95 ${
                  selectedAnswer === 'left'
                    ? isAnswerCorrect
                      ? 'bg-emerald-50 border-emerald-500 text-emerald-700 ring-4 ring-emerald-200/70 scale-105'
                      : 'bg-rose-50 border-rose-400 text-rose-600 ring-2 ring-rose-200'
                    : 'bg-white hover:bg-sky-50 border-slate-200 hover:border-sky-300 text-slate-700 hover:scale-102'
                }`}
              >
                {theme === 'balls' ? <CuteBall className="w-6 h-6" /> : <CuteButterfly className="w-6 h-6" />}
                <span>{nameA} ({sortedLeft.length})</span>
                {selectedAnswer === 'left' && isAnswerCorrect && (
                ...
                className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-sm sm:text-base transition-all cursor-pointer border-2 shadow-xs active:scale-95 ${
                  selectedAnswer === 'equal'
                    ? isAnswerCorrect
                      ? 'bg-emerald-50 border-emerald-500 text-emerald-700 ring-4 ring-emerald-200/70 scale-105'
                      : 'bg-rose-50 border-rose-400 text-rose-600 ring-2 ring-rose-200'
                    : 'bg-white hover:bg-amber-50 border-slate-200 hover:border-amber-300 text-slate-700 hover:scale-102'
                }`}
              >
                <span className="text-lg font-mono font-black">=</span>
                <span>თანაბარია</span>
                {selectedAnswer === 'equal' && isAnswerCorrect && (
                  <Check className="w-4 h-4 text-emerald-600 stroke-[3px]" />
                )}
              </button>
                ...
                className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-sm sm:text-base transition-all cursor-pointer border-2 shadow-xs active:scale-95 ${
                  selectedAnswer === 'right'
                    ? isAnswerCorrect
                      ? 'bg-emerald-50 border-emerald-500 text-emerald-700 ring-4 ring-emerald-200/70 scale-105'
                      : 'bg-rose-50 border-rose-400 text-rose-600 ring-2 ring-rose-200'
                    : 'bg-white hover:bg-pink-50 border-slate-200 hover:border-pink-300 text-slate-700 hover:scale-102'
                }`}
              >
                {theme === 'balls' ? <CuteFootball className="w-6 h-6" /> : <CuteFlower className="w-6 h-6" />}
                <span>{nameB} ({sortedRight.length})</span>
                {selectedAnswer === 'right' && isAnswerCorrect && (
                  <Check className="w-4 h-4 text-emerald-600 stroke-[3px]" />
                )}
              </button>
            </div>

            {/* Feedback message shown ONLY after marking */}
            {selectedAnswer && isAnswerCorrect && (
              <motion.div
                initial={{ scale: 0.85, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="flex flex-col items-center gap-1.5 bg-emerald-50 border-2 border-emerald-300 text-emerald-800 px-6 py-3 rounded-2xl shadow-xs mt-1"
              >
                <div className="flex items-center gap-2 font-black text-sm sm:text-base text-emerald-700">
                ...
                <div className="text-xs sm:text-sm font-bold text-emerald-800">
                  {sortedLeft.length > sortedRight.length ? (
                    <span>{nameA} მეტია {sortedLeft.length - sortedRight.length}-ით, ვიდრე {nameB}!</span>
                  ) : sortedLeft.length < sortedRight.length ? (
                    <span>{nameB} მეტია {sortedRight.length - sortedLeft.length}-ით, ვიდრე {nameA}!</span>
                  ) : (
                    <span>ორივე თანაბარია!</span>
                  )}
                </div>
              </motion.div>
            )}

            {selectedAnswer && isAnswerCorrect === false && (
              <motion.div
                initial={{ scale: 0.9, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="bg-rose-50 border border-rose-200 text-rose-600 font-bold text-xs sm:text-sm px-4 py-2 rounded-xl mt-1"
              >
                სცადე კიდევ ერთხელ! 🤔
              </motion.div>
            )}
