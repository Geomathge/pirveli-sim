    ...
    <div class="vertical-single-stage">
      <div class="activity-area vertical-single-activity">
        ...
        <div class="vertical-character-picker" role="group" aria-label="აირჩიე პერსონაჟი">
          ...
          <button type="button" class="primary-btn vertical-picker-practice-btn" data-next-stage aria-label="ახლა ივარჯიშე">
            <span>ახლა ივარჯიშე →</span>
          </button>
        </div>
  ...
  content.innerHTML = `
    <div class="vertical-single-stage">
      <div class="activity-area vertical-single-activity">
        ...
        <div class="vertical-character-picker" role="group" aria-label="აირჩიე პერსონაჟი">
          <button type="button" class="vertical-character-choice ${selectedCharacter === "girl" ? "selected" : ""}" data-character="girl" aria-label="აირჩიე გოგო" title="გოგო" aria-pressed="${selectedCharacter === "girl"}">
            <img src="${asset("girl-back")}" alt="გოგონა ზურგით"><span>გოგო</span>
          </button>
          <button type="button" class="vertical-character-choice ${selectedCharacter === "boy" ? "selected" : ""}" data-character="boy" aria-label="აირჩიე ბიჭი" title="ბიჭი" aria-pressed="${selectedCharacter === "boy"}">
            <img src="${asset("boy-back")}" alt="ბიჭი ზურგით"><span>ბიჭი</span>
          </button>
          <button type="button" class="primary-btn vertical-picker-practice-btn" data-next-stage aria-label="ახლა ივარჯიშე">
            <span>ახლა ივარჯიშე →</span>
          </button>
        </div>
        <div class="body-board combined-control-board single-walk-board">
        ...
        <div class="walk-tip-card">
          <strong>ჯერ სხეულზე</strong> ხელი ასწიე, შემდეგ იმავე მხარეს ერთი ნაბიჯი გადადგი.
        </div>
        <details class="hint-box" style="max-width:720px;margin:10px auto 0;">
          <summary>
...
function renderVerticalPractice() {
  ...
  const level = isUp ? upStairLevel : downStairLevel;
  const titleText = isUp ? "კურდღელი ასკუპდება ზევით" : "კურდღელი ჩამოსკუპდება ქვევით";
  const commandText = isUp ? "აასკუპე ერთი საფეხურით ზევით." : "ჩამოსკუპე ერთი საფეხურით ქვევით.";
  const completed = verticalPracticeMoves.up >= 2 && verticalPracticeMoves.down >= 2;
  content.innerHTML = `
    <div class="vertical-single-stage">
      <div class="activity-area vertical-single-activity">
        <div class="instruction child-instruction practice-command"><span class="instruction-number">2</span><div><h3>ამოძრავე კურდღელი ზევით და ქვევით.</h3><p>აირჩიე მიმართულება: ზევით ასკუპება ან ქვევით ჩამოსკუპება.</p></div>${audioButton("ამოძრავე კურდღელი ზევით და ქვევით. თითო დაჭერაზე გაიარე ერთი საფეხური.")}</div>

        <div class="vertical-character-picker" role="group" aria-label="აირჩიე მიმართულება">
          <button type="button" class="vertical-character-choice ${isUp ? "selected" : ""}" data-rabbit-direction="up" aria-pressed="${isUp}">
            <img src="assets/objects/rabbit-hop.png?v=${motionAssetVersion}" alt="ზევით"><span>ზევით (ასკუპება)</span>
          </button>
          <button type="button" class="vertical-character-choice ${!isUp ? "selected" : ""}" data-rabbit-direction="down" aria-pressed="${!isUp}">
            <img src="assets/objects/rabbit-land.png?v=${motionAssetVersion}" alt="ქვევით"><span>ქვევით (ჩამოსკუპება)</span>
          </button>
          <button type="button" class="primary-btn vertical-picker-practice-btn" id="stairPracticeCheck" aria-label="მოკლე შემოწმება">
            <span>მოკლე შემოწმება →</span>
          </button>
        </div>
        ...
        <section class="stair-demo-card single-stair-card" data-stair-demo="${direction}">
          <h4>${titleText}</h4>
          <div class="stair-demo-action">
            ${staircaseMarkup(level, "practiceStairAvatar", direction)}
            <button class="stair-control ${direction === "up" ? "stair-up" : "stair-down"}" type="button" data-stair-move="${direction}" data-practice-stair-move="${direction}" aria-label="${commandText}">
        ...
        <details class="hint-box" style="max-width:720px;margin:10px auto 0;">
          ...
          <div class="hint-content">
            ↑ კურდღელს ზევით ასკუპებს. ↓ კურდღელს ქვევით ჩამოსკუპებს.
          </div>
        </details>
        <div class="feedback" id="feedback" hidden></div>
      </div>
  ...
  content.querySelectorAll("[data-practice-stair-move]").forEach(button => button.addEventListener("click", () => {
    ...
    verticalPracticeMoves[chosen] = (verticalPracticeMoves[chosen] || 0) + 1;
    ...
    } else if (verticalPracticeMoves[chosen] >= 2) {
      const otherDir = chosen === "up" ? "down" : "up";
      if ((verticalPracticeMoves[otherDir] || 0) < 2) {
        feedback(true, `ყოჩაღ! ახლა აირჩიე „${otherDir === "up" ? "ზევით" : "ქვევით"}“ და იქაც ივარჯიშე.`);
      }
    }
  }));
  content.querySelector("#stairPracticeCheck").addEventListener("click", () => setStage("check"));
  announce(isUp ? "კურდღელი ასკუპდება ზევით." : "კურდღელი ჩამოსკუპდება ქვევით.");
}
