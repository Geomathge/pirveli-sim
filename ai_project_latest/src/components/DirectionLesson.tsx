...
export default function DirectionLesson({mode,onNavigate}:{mode:'horizontal'|'vertical',onNavigate:(mode:'horizontal'|'vertical'|'comparison')=>void}){
 ...
 useEffect(()=>{const receive=(e:MessageEvent)=>{if(e.source!==ref.current?.contentWindow)return;if(e.data?.type==='direction-height'&&Number.isFinite(e.data.height))setHeight(Math.max(500,e.data.height+30));if(e.data?.type==='direction-navigate'&&['horizontal','vertical','comparison'].includes(e.data.mode))onNavigate(e.data.mode)};window.addEventListener('message',receive);return()=>window.removeEventListener('message',receive)},[onNavigate]);
