...
function BallTile({ kind, paired, extra }: { kind: 'basketball' | 'football'; paired?: boolean; extra?: boolean }) {
  return (
    <div className={`w-[80px] h-[80px] sm:w-[94px] sm:h-[94px] rounded-2xl border-2 flex items-center justify-center shrink-0 transition-all ${
      extra
      ...
      {kind === 'basketball'
        ? <CuteBall className="w-[60px] h-[60px] sm:w-[70px] sm:h-[70px]" />
        : <CuteFootball className="w-[60px] h-[60px] sm:w-[70px] sm:h-[70px]" />}
    </div>
  );
}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-4 flex-1">
                {Array.from({ length: BASKETBALLS }).map((_, idx) => (
                  <BallTile key={`basketball-${idx}`} kind="basketball" paired={idx < PAIRABLE && paired.includes(idx)} extra={idx >= PAIRABLE} />
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-4 flex-1">
                {Array.from({ length: BASKETBALLS }).map((_, idx) => idx < FOOTBALLS
                  ? <BallTile key={`football-${idx}`} kind="football" paired={paired.includes(idx)} />
                  : <div key={`football-${idx}`} className="w-[80px] h-[80px] sm:w-[94px] sm:h-[94px] rounded-2xl border-2 border-dashed border-slate-100 bg-white shrink-0" />)}
              </div>
            </div>
          </div>
        </div>
